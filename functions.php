...
* URI: https://studio.bio/themes/grate
*------------------------------------
*
...
*/
// require_once( 'library/grate.php' );
...
add_action( 'after_setup_theme', 'grateness' );
...
function grateness() {
    ...
    load_theme_textdomain( 'grate', get_template_directory() . '/library/translation' );

    // launching operation cleanup
    add_action( 'init', 'grate_head_cleanup' );

    // A better title
    add_filter( 'wp_title', 'rw_title', 10, 3 );

    // remove WP version from RSS
    add_filter( 'the_generator', 'grate_rss_version' );

    // remove pesky injected css for recent comments widget
    add_filter( 'wp_head', 'grate_remove_wp_widget_recent_comments_style', 1 );

    // clean up comment styles in the head
    add_action( 'wp_head', 'grate_remove_recent_comments_style', 1 );

    // clean up gallery output in wp
    add_filter( 'gallery_style', 'grate_gallery_style' );

    // enqueue base scripts and styles
    add_action( 'wp_enqueue_scripts', 'grate_scripts_and_styles', 999 );

    // launching this stuff after theme setup
    grate_theme_support();

    // adding sidebars to Wordpress
    add_action( 'widgets_init', 'grate_register_sidebars' );

    // cleaning up random code around images
    add_filter( 'the_content', 'grate_filter_ptags_on_images' );

    // cleaning up excerpt
    add_filter( 'excerpt_more', 'grate_excerpt_more' );
...
// Thumbnail sizes
add_image_size( 'grate-image-600', 600, 600, true );
add_image_size( 'grate-image-300', 300, 300, true );
add_image_size( 'grate-image-300', 150, 150, true );
...
<?php the_post_thumbnail( 'grate-image-300' ); ?>
for the 600 x 150 image:
<?php the_post_thumbnail( 'grate-image-600' ); ?>

You can change the names and dimensions to whatever
you like. Enjoy!
*/

add_filter( 'image_size_names_choose', 'gratecustom_image_sizes' );

function grate_custom_image_sizes( $sizes ) {

    return array_merge( $sizes, array(

        'grate-image-600' => __('600px by 600px', 'grate'),
        'grate-image-300' => __('300px by 300px', 'grate'),
        'grate-image-150' => __('150px by 150px', 'grate'),
...
function grate_register_sidebars() {

	register_sidebar( array(

		'id' => 'sidebar1',
		'name' => __( 'Sidebar 1', 'grate' ),
		'description' => __( 'The first (primary) sidebar.', 'grate' ),
	...
	register_sidebar( array(
		...
		'id' => 'sidebar2',
		'name' => __( 'Sidebar 2', 'grate' ),
		'description' => __( 'The second (secondary) sidebar.', 'grate' ),
		'before_widget' => '<div id="%1$s" class="widget %2$s">',
		'after_widget' => '</div>',
		'before_title' => '<h4 class="widgettitle">',
		'after_title' => '</h4>',
...
// Comment Layout
function grate_comments( $comment, $args, $depth ) {
        ...
        <article  class="cf">
            ...
            <header class="comment-author vcard">
                ...
                */
                ...
                <?php printf(__( '<cite class="fn">%1$s</cite> %2$s', 'grate' ), get_comment_author_link(), edit_comment_link(__( '(Edit)', 'grate' ),'  ','') ) ?>

                <time datetime="<?php echo comment_time('Y-m-j'); ?>">

                    <a href="<?php echo htmlspecialchars( get_comment_link( $comment->comment_ID ) ) ?>"><?php comment_time(__( 'F jS, Y', 'grate' )); ?> </a>
            ...
            <?php if ($comment->comment_approved == '0') : ?>
                ...
                <div class="alert alert-info">
                    ...
                    <p><?php _e( 'Your comment is awaiting moderation.', 'grate' ) ?></p>
...
// add_action( 'wp_enqueue_scripts', 'grate_fonts' );

// function grate_fonts() {
...
function grate_head_cleanup() {
    ...
    add_filter( 'style_loader_src', 'grate_remove_wp_ver_css_js', 9999 );

    // remove WP version from scripts
    add_filter( 'script_loader_src', 'grate_remove_wp_ver_css_js', 9999 );

} /* end template head cleanup */


// remove WP version from RSS
function grate_rss_version() {
...
// remove WP version from scripts
function grate_remove_wp_ver_css_js( $src ) {
...
function grate_remove_wp_widget_recent_comments_style() {
...
function grate_remove_recent_comments_style() {
...
function grate_gallery_style($css) {
...
function grate_scripts_and_styles() {
    ...
    if ( !is_admin() ) {
        ...
        wp_enqueue_style( 'grate-stylesheet', get_theme_file_uri() . '/library/css/style.css', array(), '', 'all' );

        // ie-only style sheet
        wp_enqueue_style( 'grate-ie-only', get_theme_file_uri() . '/library/css/ie.css', array(), '' );

        // comment reply script for threaded comments
        if ( is_singular() AND comments_open() AND ( get_option('thread_comments') == 1 )) { wp_enqueue_script( 'comment-reply' ); }

        // adding scripts file in the footer
        wp_enqueue_script( 'grate-js', get_theme_file_uri() . '/library/js/scripts.js', array( 'jquery' ), '', true );

        $wp_styles->add_data( 'grate-ie-only', 'conditional', 'lt IE 9' ); // add conditional wrapper around ie stylesheet

        // grateate extra scripts. Uncomment to use. Or better yet, copy what you need to the main scripts folder or on the page(s) you need it
        // wp_enqueue_script( 'grae-extra-js', get_theme_file_uri() . '/library/js/extras/extra-scripts.js', array( 'jquery' ), '', true );
...
*/
// add_action( 'wp_default_scripts', 'grate_dequeue_jquery_migrate' );

// function grate_dequeue_jquery_migrate( $scripts ) {
...
function grate_filter_ptags_on_images( $content ) {
...
function grate_excerpt_more( $more ) {

    global $post;

     // edit here if you like
    return '...  <a class="excerpt-read-more" href="'. get_permalink( $post->ID ) . '" title="'. __( 'Read ', 'grate' ) . esc_attr( get_the_title( $post->ID ) ).'">'. __( 'Read more &raquo;', 'grate' ) .'</a>';
...
function grate_theme_support() {
    ...
    add_theme_support( 'custom-header', array(
        ...
        'header-text'            => true,
        'uploads'                => true,
        'wp-head-callback'       => 'grate_style_header',
    ...
    register_nav_menus( array(
          ...
          'main-nav' => __( 'The Main Menu', 'grate' ),   // main nav in header
...
add_action( 'customize_register', 'grate_register_theme_customizer' );

function grate_register_theme_customizer( $wp_customize ) {
    ...
    $wp_customize->get_section( 'title_tagline' )->title = __( 'Site Name and Description', 'grate' );  
    $wp_customize->get_control( 'blogname' )->label = __( 'Site Name', 'grate' );  
    $wp_customize->get_control( 'blogdescription' )->label = __( 'Site Description', 'grate' );  
    $wp_customize->get_setting( 'blogname' )->transport = 'postMessage';
    $wp_customize->get_setting( 'blogdescription' )->transport  = 'postMessage';

    // Customize the Front Page Settings
    $wp_customize->get_section( 'static_front_page' )->title = __( 'Homepage Preferences', 'grate' );
    $wp_customize->get_section( 'static_front_page' )->priority = 20;
    $wp_customize->get_control( 'show_on_front' )->label = __( 'Choose Homepage Preference:', 'grate' );  
    $wp_customize->get_control( 'page_on_front' )->label = __( 'Select Homepage:', 'grate' );  
    $wp_customize->get_control( 'page_for_posts' )->label = __( 'Select Blog Homepage:', 'grate' );  

    // Customize Background Settings
    $wp_customize->get_section( 'background_image' )->title = __( 'Background Styles', 'grate' );  
    $wp_customize->get_control( 'background_color' )->section = 'background_image'; 

    // Customize Header Image Settings  
    $wp_customize->add_section( 'header_text_styles' , array(

        'title'      => __( 'Header Text Styles', 'grate' ), 
        'priority'   => 30
...
add_action( 'customize_preview_init', 'grate_customizer_scripts' );

function grate_customizer_scripts() {

    wp_enqueue_script( 'grate_theme_customizer', get_template_directory_uri() . '/library/js/theme-customizer.js', array( 'jquery', 'customize-preview' ), '', true);

    // register customizer stylesheet
    wp_register_style( 'grate-customizer', get_theme_file_uri() . '/library/css/customizer.css', array(), '', 'all' );
    wp_enqueue_style( 'grate-customizer' );

}


// Callback function for updating header styles
function grate_style_header() {
...
function grate_related_posts() {
    ...
    if( $tags ) {
        ...
        if( $related_posts ) {
            ...
            <?php echo '<li class="no_related_post">' . __( 'No Related Posts Yet!', 'grate' ) . '</li>'; ?>
...
add_filter( 'body_class', 'grate_body_class' );

function grate_body_class( $classes ) {
...
*
*/
add_action( 'admin_print_footer_scripts', 'grate_custom_quicktags' );

function grate_custom_quicktags() {
...
// add_action( 'wp_enqueue_scripts', 'grate_load_dashicons' );

// function grate_load_dashicons() {
...
if ( ! function_exists( 'grate_posted_on' ) ) :
/**
 * Prints HTML with meta information for the current post-date/time and author.
 */
function grate_posted_on() {
    ...
    echo '<span class="posted-on">' . grate_time_link() . '</span><span class="byline"> ' . $byline . '</span>';
...
if ( ! function_exists( 'grate_time_link' ) ) :
/**
 * Gets a nicely formatted string for the published date.
 */
function grate_time_link() {
    ...
    return sprintf(
        ...
        __( '<span class="screen-reader-text">Posted on</span> %s', 'grate' ),
